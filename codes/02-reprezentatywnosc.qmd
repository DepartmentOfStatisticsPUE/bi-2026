...
Porównajmy rozkłady zmiennych pomocniczych -- różnice wskazują na autoselekcję w CBOP:
...
Łączymy oba zbiory w jeden, dodając zmienną `source` wskazującą przynależność do JVS (populacja referencyjna) lub CBOP (próba nielosowa):
...
Interpretacja: kolumna `Diff.Un` to SMD przed jakąkolwiek korekcją. Wartości dalekie od 0 wskazują zmienne, w których CBOP różni się od JVS -- te zmienne napędzają autoselekcję.

## Ważenie propensity score (`WeightIt`)

Estymujemy propensity score, czyli $P(\text{CBOP} \mid \mathbf{X})$ -- prawdopodobieństwo, że firma znajdzie się w CBOP, a nie w JVS, biorąc pod uwagę jej cechy. Odwrotności tych prawdopodobieństw stają się wagami korygującymi:
